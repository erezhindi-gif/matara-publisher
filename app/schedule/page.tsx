      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm transition-colors ${view === "week" ? "bg-blue-600" : "bg-gray-200 hover:bg-gray-300"}`}
            >
              שבועי
            </button>
            <button
              onClick={() => setView("list")}
              className={`px-4 py-2 rounded-xl text-sm transition-colors ${view === "list" ? "bg-blue-600" : "bg-gray-200 hover:bg-gray-300"}`}
            >
              רשימה
            </button>
          </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-7 gap-2">
            {DAYS.map((day, i) => (
              <div key={i} className="bg-gray-100 border border-gray-200 rounded-xl p-3 min-h-[200px]">
                <div className="text-xs font-semibold text-gray-700 mb-3 text-center">{day}</div>
                <div className="space-y-2">
                  {byDay[i].map((c) => (
                    <div
                      key={c.id}
                      className="text-xs p-2 rounded-lg bg-gray-200 border border-gray-300"
                    >
          ...
          <div className="space-y-3">
            {campaigns.map((c) => (
              <div key={c.id} className="bg-gray-100 border border-gray-200 rounded-2xl p-4 flex items-center gap-4">
