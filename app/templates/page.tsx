      ...
      <div className="max-w-4xl mx-auto">
        ...
        {showNew && (
          <div className="bg-gray-900 border border-blue-500 rounded-2xl p-5 mb-6">
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-sm text-gray-400 mb-1">שם התבנית</label>
                <input
              ...
              <div className="flex gap-3">
                ...
                <button onClick={createTemplate} disabled={!newName.trim() || saving} className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-700 rounded-xl p-3 font-semibold transition-colors">
        ...
        {!loading && templates.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {templates.map((t) => (
              <Link key={t.id} href={`/templates/${t.id}`} className="block bg-gray-900 border border-gray-800 rounded-2xl p-5 hover:border-gray-600 transition-all">
                <div className="flex items-start justify-between mb-2">
                  <h2 className="font-semibold text-lg">{t.name}</h2>
                  <span className="text-xs text-gray-500 bg-gray-800 px-2 py-1 rounded-full">{t.business.name}</span>
                </div>
                ...
                {t.groups.length > 0 && (
                  <div className="mt-3 space-y-1">
                    ...
                    {t.groups.length > 3 && (
                      <div className="text-xs text-gray-600">+{t.groups.length - 3} נוספות</div>
                    )}
                  </div>
                )}
              </Link>
